...
// ----------------------------------------------------------------------------
// RemoteAgent Implementation (Inlined to resolve Linker Issues)
// ----------------------------------------------------------------------------
class RemoteAgent {
public:
    struct Command {
...
int main(int argc, char* argv[]) {
    ...
    while (!done) {
        ...
        // Poll Logs
        if (selected_pid > 0 && now - lastLogPoll > 50) { // Faster polling
...

        {
            ...
            if (ImGui::BeginTabBar("MainTabs")) {
                ...
                if (ImGui::BeginTabItem("Console")) {
                    ...
                    // Console Input
                    static char consoleInput[1024] = "";
                    ImGui::PushItemWidth(-1);
                    if (ImGui::InputText("##ConsoleInput", consoleInput, IM_ARRAYSIZE(consoleInput), ImGuiInputTextFlags_EnterReturnsTrue)) {
                        if (consoleInput[0] != 0 && selected_pid > 0) {
                            RemoteAgent::Get().Send(selected_pid, CMD_RUN_SCRIPT, consoleInput);
                            console_log += "> " + std::string(consoleInput) + "\n";
                            consoleInput[0] = 0;
                            // Keep focus
